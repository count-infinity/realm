...
class ScriptFunctions:
    ...
    def __init__(
        ...
        # Output buffer for commands generated by scripts
        self.command_queue: list[tuple[GameObject, str]] = []
    ...
    def get(self, spec: str) -> GameObject | None:
        ...
        if spec.startswith('#'):
            obj_id = spec[1:]
            return self._persistence._object_cache.get(obj_id)

        # Name lookup in cache
        spec_lower = spec.lower()
        for obj in self._persistence._object_cache.values():
    ...
    def pemit(self, target: GameObject | str, message: str) -> None:
        ...
        This queues a pemit command rather than sending directly.
        """
        if isinstance(target, str):
            target_obj = self.get(target)
            if target_obj:
                self.command_queue.append((target_obj, f"@pemit {target_obj.id}={message}"))
        else:
            self.command_queue.append((target, f"@pemit {target.id}={message}"))

    def remit(self, room: GameObject | str, message: str) -> None:
        """Emit a message to all in a room."""
        if isinstance(room, str):
            room_obj = self.get(room)
            if room_obj:
                self.command_queue.append((room_obj, f"@remit {room_obj.id}={message}"))
        else:
            self.command_queue.append((room, f"@remit {room.id}={message}"))

    def oemit(self, exclude: GameObject | str, message: str) -> None:
        """Emit to room, excluding one object."""
        if self.executor and self.executor.location:
