...
class TestAttributeInheritance:
    """Test attribute resolution with parent chain."""

    def test_local_attribute(self):
        """Local attributes are found first."""
        obj = GameObject("test")
        obj.db.health = 100
        assert obj.resolve_attr('health') == 100

    def test_parent_attribute(self):
        """Attributes inherit from parent."""
        parent = GameObject("parent")
        parent.db.health = 100

        child = GameObject("child", parent=parent)
        assert child.resolve_attr('health') == 100

    def test_local_overrides_parent(self):
        """Local attributes override parent."""
        parent = GameObject("parent")
        parent.db.health = 100
        ...
        assert child.resolve_attr('health') == 50
        assert parent.resolve_attr('health') == 100

    def test_deep_inheritance(self):
        """Attribute inheritance works through multiple levels."""
        grandparent = GameObject("grandparent")
        grandparent.db.value = "from grandparent"

        parent = GameObject("parent", parent=grandparent)
        child = GameObject("child", parent=parent)

        assert child.resolve_attr('value') == "from grandparent"

    def test_default_value(self):
        """resolve_attr returns default for missing attributes."""
        obj = GameObject("test")
        assert obj.resolve_attr('missing', 'default') == 'default'
        assert obj.resolve_attr('missing') is None

    def test_circular_parent_protection(self):
        """Circular parent references don't cause infinite loops."""
        obj1 = GameObject("obj1")
        obj2 = GameObject("obj2", parent=obj1)
        obj1.parent = obj2  # Create cycle

        # Should not hang or crash
        result = obj1.resolve_attr('missing', 'default')
