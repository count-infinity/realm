...
BUILD_177 = [
    ...
    "@set Warden/arm = t = create_obj('a sentence timer', ['thing','jail_timer'], 'The Holding Cell'); set_attr(t, 'prisoner', arg0); set_attr(t, 'warden', me.id); set_attr(t, 'on_expire', \"p = get('#'+str(V('prisoner'))); w = get('#'+str(V('warden'))); (remove_tag(p,'jailed'), teleport_obj(p,'The Precinct'), pemit(p,'The cell door clicks open. Time served.'), set_attr(w,'log', ((get_attr(w,'log') or []) + ['auto-released ' + name(p)])[-50:])) if p and w else None\"); expire(t, int(arg1) * 60)",
    "@set Warden/cmd_jail = $jail * = *: p = get(trim(arg0)); mins = int(trim(arg1)) if trim(arg1).isdigit() else 5; (pemit(enactor,'Only staff may work the Warden.') if not has_tag(enactor,'admin') else (pemit(enactor,f'No one named {trim(arg0)} to jail.') if not (p and has_tag(p,'player')) else (add_tag(p,'jailed'), teleport_obj(p,'The Holding Cell'), pemit(p,f'You are hauled off to the Holding Cell. Sentence: {mins} minute(s).'), set_attr(me,'log', ((V('log') or []) + [f'{name(enactor)} jailed {name(p)} ({mins}m)'])[-50:]), eval_attr(me,'arm', p.id, mins), pemit(enactor,f'Jailed {name(p)} for {mins} minute(s).'))))",
    "@set Warden/cmd_free = $free *: p = get(trim(arg0)); (pemit(enactor,'Only staff may work the Warden.') if not has_tag(enactor,'admin') else (pemit(enactor, f'{trim(arg0)} is not currently jailed.') if not (p and has_tag(p,'jailed')) else (remove_tag(p,'jailed'), teleport_obj(p,'The Precinct'), pemit(p,'You are released early. Stay out of trouble.'), [destroy_obj(t) for t in contents(get('The Holding Cell')) if has_tag(t,'jail_timer') and get_attr(t,'prisoner') == p.id], set_attr(me,'log', ((V('log') or []) + [f'{name(enactor)} freed {name(p)}'])[-50:]), pemit(enactor,f'Freed {name(p)}.'))))",
    "@set Warden/cmd_jaillog = $jail log: (pemit(enactor,'Only staff may work the Warden.') if not has_tag(enactor,'admin') else (pemit(enactor,'The blotter is empty.') if not V('log') else [pemit(enactor, ln) for ln in V('log')[-10:]]))",
]
...
BUILD_179 = [
    ...
    "@set Approvals Desk/cmd_pending = $pending: q = search_world(tag='unapproved'); (pemit(enactor,'Only staff may review arrivals.') if not has_tag(enactor,'admin') else (pemit(enactor,'The approval queue is empty.') if not q else [pemit(enactor,f'- {name(p)} (#{str(p.id)[:8]})') for p in q]))",
    "@set Approvals Desk/cmd_approve = $approve *: p = get(trim(arg0)); (pemit(enactor,'Only staff may clear arrivals.') if not has_tag(enactor,'admin') else (pemit(enactor, f'{trim(arg0)} is not awaiting approval.') if not (p and has_tag(p,'unapproved')) else (remove_tag(p,'unapproved'), add_tag(p,'approved'), pemit(p,'Your character has been approved. Welcome aboard — the concourse is open to you.'), set_attr(me,'log', ((V('log') or []) + [f'{name(enactor)} approved {name(p)}'])[-50:]), pemit(enactor,f'Approved {name(p)}.'))))",
    "@set Approvals Desk/cmd_reject = $reject * = *: p = get(trim(arg0)); (pemit(enactor,'Only staff may clear arrivals.') if not has_tag(enactor,'admin') else (pemit(enactor, f'{trim(arg0)} is not awaiting approval.') if not (p and has_tag(p,'unapproved')) else (pemit(p,f'Your character needs work before approval: {escape(trim(arg1))}'), set_attr(me,'log', ((V('log') or []) + [f'{name(enactor)} bounced {name(p)}: {trim(arg1)}'])[-50:]), pemit(enactor,f'Sent {name(p)} back with notes.'))))",
]
