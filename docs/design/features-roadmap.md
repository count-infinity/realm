...
| **Wilderness** (procgen coordinate cells too vast to hand-build) | 🔶 Trigger shipped, feature pending | Same materialize-on-demand primitive as instances, keyed by `(x,y)` instead of PC. Its trigger is **`event:on_fail`** (shipped 2026-07-12) — a dead-end exit whose `ON_FAIL` get-or-creates the neighbor cell and moves the walker in. Remaining: a map-provider softcode lib (`name/desc/terrain` per coordinate) + the get-or-create-cell helper over the `ephemeral` flag. |
