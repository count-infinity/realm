...
@set bio-scanner/render = p = get('#' + arg0); bar = repeat('=', 40); hp = int(get_attr(p, 'hp', 0)); mhp = max(1, int(get_attr(p, 'max_hp', 1))); filled = max(0, min(10, hp * 10 // mhp)); hpbar = '[' + repeat('#', filled) + repeat('-', 10 - filled) + ']'; sk = [s for s in get_attr(me, 'skills', '').split() if get_attr(p, 'skill_' + s) != None]; cond = [t for t in tags(p) if t in ['wounded', 'bleeding', 'resting', 'starving', 'unconscious', 'encumbered', 'restrained']]; result = '\n'.join([bar, '  ' + name(p) + '  --  ' + str(get_attr(p, 'template', 'unregistered')), bar, '  ST ' + str(get_attr(p, 'strength', 10)) + '    DX ' + str(get_attr(p, 'dexterity', 10)) + '    IQ ' + str(get_attr(p, 'intelligence', 10)) + '    HT ' + str(get_attr(p, 'health', 10)), '  HP ' + hpbar + ' ' + str(hp) + '/' + str(mhp) + '     Dodge ' + str(get_attr(p, 'dodge', 8)), '  CP ' + str(get_attr(p, 'character_points', 0)), '  Skills: ' + (', '.join([s + '-' + str(get_attr(p, 'skill_' + s)) for s in sk]) or 'none trained'), '  Status: ' + (', '.join(cond) or 'nominal'), bar])
```
