...
- [x] 61. **Patrolling guard** [now] — walks a fixed route, pauses, reacts to open doors. *(patrol behavior, waypoints, ON_OPEN reactions)*
- [x] 62. **Aggressive mob** [now] — attacks on sight based on faction standing. *(aggressive behavior, dispositions, start_combat)*
...
- [x] 65. **Pet** [now] — follows its owner between rooms and responds to simple orders. *(db.following, $-command whitelists, ownership)*
- [x] 66. **Puppet** [now] — a second body a player can possess and speak/act through. *(control locks, @force, puppet output forwarding)*
...
- [x] 69. **Trainer NPC** [now] — teaches skills for money with prerequisites and limits. *(CP economy, $train wrappers, admin-owned masters and authority)*
- [x] 70. **Pickpocket NPC** [now] — attempts theft with contested rolls; can be caught. *(contest(), move_to, the controls() authority model)*
...
- [x] 72. **NPC reaction emotes** [now] — NPC visibly reacts to says, emotes, and weapon-drawing. *(^listen + ON_WIELD triggers, now()-based cooldowns)*
- [x] 73. **Boss with phases** [now] — scripted combat behavior that changes at HP thresholds. *(ON_HITPRCNT, combat strategies, behavior swapping)*

## 7. Communication Systems (74–85)

- [x] 74. **Custom channel** [now] — create a channel with history, muting, and per-channel aliases. *(world-zone master $-commands, pemit fan-out, history attrs)*
- [x] 75. **In-game mail** [now] — persistent mail with CC and item attachments. *(post-office object storage, escrow inventory, ON_CONNECT notices)*
...
- [x] 88. **Player-run shop stalls** [now] — rentable stalls where players price their own goods. *(delegated vendor attrs, on_tick rent, escrow)*
...
- [x] 90. **Pawn shop** [now] — sells anything back at a percentage with buyback window. *(db.value valuation, expire() buyback windows)*
- [x] 91. **Lottery** [now] — buy numbered tickets; scheduled global drawing pays the pot. *(ticket items, on_tick drawings, pot transfers)*
...
- [x] 93. **Housing rent** [now] — periodic rent with warnings, then lockout/eviction. *(on_tick billing, grace attrs, set_lock eviction)*
- [x] 94. **Job board** [now] — NPC-posted paid tasks claimed and verified automatically. *(posting attrs, ON_GIVE/ON_PAYMENT validation, payouts)*
- [x] 95. **Item durability & repair** [now] — wear from use, repair costs as a money sink. *(durability attrs, zone-master ON_DAMAGE bookkeeping, $repair)*
- [x] 96. **Secure player trade** [now] — both parties stage items/money and confirm atomically. *(escrow objects, dual prompt() confirms, one-script commit)*
- [x] 97. **Barter NPC** [now] — trades item-for-item against a want-list instead of cash. *(want-list attrs, ON_RECEIVE matching)*

## 9. Games & Recreation (98–108)

- [x] 98. **Dice roller** [now] — `roll 3d6` with success/margin interpretation vs. a skill. *(roll(), margin_under(), the resolution primitives)*
- [x] 99. **Card deck** [now] — shuffle, deal, hold hands only their owner can see. *(list attrs, rand shuffles, pemit-private state)*
- [x] 100. **Poker table** [now] — multi-player hold'em with betting rounds and pot. *(sandboxed-Python state machines, prompt() turns, hidden info)*
- [x] 101. **Chess board** [now] — two-player board rendered in ASCII with move validation. *(eval_attr render helpers, grid attrs, sandboxed validation)*
- [x] 102. **Trivia host NPC** [now] — timed rounds of questions loaded from a data file. *(pack/attr question data, prompt() answer windows, scoring)*
- [x] 103. **Rock-paper-scissors** [now] — challenge with optional wagers and simultaneous reveal. *(dual prompt() secrets, escrowed bets, reveal remit)*
- [x] 104. **Scavenger hunt** [now] — staff-set list of items/locations to find, with leaderboard. *(ON_GET/ON_ARRIVE detection, registry attrs)*
- [x] 105. **NPC races & betting** [now] — scheduled races with odds and payouts. *(on_tick simulation, odds attrs, betting-book objects)*
- [x] 106. **Arm wrestling** [now] — quick contest of ST with crowd-pleasing play-by-play. *(contest(), remit spectacle, wagers)*
- [x] 107. **Dart board** [now] — throw for score; doubles as a DX practice minigame. *(skill_check margins, CP practice awards)*
- [x] 108. **Casino floor** [now] — chips exchange, house bank, and multiple table games tied together. *(composing prior builds into a venue)*

## 10. Combat & Conflict Extensions (109–120)

- [x] 109. **Cover system** [now] — take cover behind room fixtures for ranged defense bonuses. *(the engine cover maneuver, cover-tagged fixtures)*
...
- [x] 111. **Grenades** [now] — fuse timers, throwing skill, scatter on a miss. *(wait() fuses, room-loop damage(), rand scatter over exits())*
- [x] 112. **Non-lethal takedowns** [now] — subdue instead of kill; captives and restraints. *(engine unconsciousness, ON_HITPRCNT surrender, restraint wards)*
- [x] 113. **Dueling system** [now] — formal consent-based PvP with stakes and a referee. *(consent prompt()s, start_combat, stakes escrow)*
- [x] 114. **Bounty board** [now] — post bounties on characters; claims verified on defeat. *(ON_DEATH verification, escrow payouts)*
- [x] 115. **Arena with spectators** [now] — fight pit that broadcasts blow-by-blow to the stands. *(recorder relays: ON_ATTACK/ON_DAMAGE → remit to stands)*
- [ ] 116. **Called shots** [small] — target hit locations with penalties and effects. *(gap G9: hit-location maneuver in the GURPS ruleset)*
- [x] 117. **Armor degradation** [now] — armor absorbs damage but wears out and needs repair. *(ON_DAMAGE bookkeeping, DR attrs, repair sinks)*
- [x] 118. **Bleeding & first aid** [now] — wounds bleed over time; First Aid skill stabilizes. *(damage_over_time behavior, the firstaid command)*
- [x] 119. **NPC morale** [now] — mobs check morale, flee, or surrender when losing. *(ON_HITPRCNT behavior swaps, fleeing behavior, dispositions)*
- [x] 120. **Combat replay log** [now] — per-fight log reviewable afterwards by participants. *(recorder objects, event-appended log attrs, $replay)*

## 11. Crafting & Resources (121–131)

- [x] 121. **Gathering nodes** [now] — mineable ore/salvage spots that deplete and respawn. *(depletion attrs, on_tick respawn, yield margins)*
- [x] 122. **Recipe crafting** [now] — combine ingredients at a bench with a skill roll. *(recipe validation, destroy/create, margin quality)*
- [x] 123. **Refining chain** [now] — ore → ingot → parts, each stage a different station. *(multi-stage pipelines, tag-gated stations)*
- [x] 124. **Salvage & disassembly** [now] — break items into components based on what they are. *(reverse recipes, component tables by tag)*
- [x] 125. **Quality tiers** [now] — craft margin determines fine/good/shoddy output with stat effects. *(margin-driven output attrs)*
- [x] 126. **Blueprint items** [now] — learnable schematics that unlock recipes when studied. *(ON_USE unlocks, known-list attrs)*
- [x] 127. **Crafting stations** [now] — recipes requiring specific benches/tools present. *(environment tag checks in recipes)*
- [x] 128. **Hydroponics farming** [now] — plant, water, and harvest crops growing in real time. *(on_tick growth stages, stage-swapped descs)*
- [x] 129. **Cooking with buffs** [now] — meals granting temporary bonuses; spoilage. *(modifier_effect consumables, decay spoilage)*
- [x] 130. **Fishing** [now] — cast, wait, hook with a timing/skill minigame and catch tables. *(wait() + prompt() timing windows, catch tables)*
- [x] 131. **Chemistry & poisons** [now] — mix reagents into acids, medicines, and toxins safely or not. *(risky recipes, failure effects, skill prereqs)*
...
- [x] 143. **XP spending** [now] — earn and spend points to improve skills with time-based limits. *(the CP economy: points/improve, now()-based limits)*

## 13. Time, Scheduling & Automation (144–153)

- [x] 144. **Game calendar & clock** [now] — custom epoch, month names, and time conversion utils. *(softcode clocks from now(), calendar tables in attrs)*
- [x] 145. **Scheduled world events** [now] — cron-style events (market day, meteor shower). *(on_tick cron checks, event-registry attrs)*
- [x] 146. **Item decay** [now] — food rots, corpses decompose, litter is swept up. *(decay behavior, expire()/ON_EXPIRE, @foreach sweeps)*
- [x] 147. **Zone repop** [now] — areas reset spawns and states on a cycle, skipping occupied rooms. *(zone_reset behavior, reset_spec, ON_RESET)*
- [x] 148. **Delayed actions** [now] — "the fuse burns down…" — patterns for one-off future effects. *(wait()/cancel_wait idioms, the persistence caveat)*
- [x] 149. **Maintenance sweeper** [now] — nightly script that finds orphans and cleans junk data. *(on_tick housekeeping, search_world queries, destroy_obj)*
- [x] 150. **Global countdown events** [now] — server-wide announcements at T-minus intervals. *(wait() chains, remit loops over all rooms)*
- [x] 151. **Business hours** [now] — shops and services only operate at certain game times. *(clock-driven behavior attach/detach, [[...]] closed states)*
- [x] 152. **Reboot-surviving timers** [now] — timers that resume correctly after server restart. *(db timestamps + on_tick vs in-memory wait(); expire() persists)*
- [x] 153. **Time scaling** [now] — running game time faster than real time and what it breaks. *(clock factors, world tick vs beats, combat pace)*

## 14. Movement & Transportation (154–164)

- [x] 154. **Elevator** [now] — multi-floor car with call buttons and moving-room illusion. *(exit relinking via db.destination, state-driven connections)*
- [x] 155. **Drivable vehicle** [now] — enterable rover you steer through the world from inside. *(vehicle-as-container-room, $drive traversal, [[...]] outside view)*
- [x] 156. **Scheduled shuttle** [now] — transport that departs on a timetable along a route. *(on_tick timetables, boarding windows)*
- [x] 157. **Teleporter pads** [now] — networked pads with destination selection and effects. *(teleport_obj, registry attrs, arrival remits)*
- [x] 158. **Mounts** [now] — ride an animal/bike for speed; dismount rules. *(containment or following, $ride/$dismount)*
- [x] 159. **Group travel** [now] — follow/lead so parties move as one. *(the built-in follow/party system, follower cascade)*
...
- [x] 161. **Travel time** [now] — long-distance moves take real seconds with progress messages. *(ward-blocked exits + wait() progress + teleport arrival)*
- [x] 162. **Tracking** [now] — follow footprints left by earlier travelers with Tracking rolls. *(ON_LEAVE evidence objects, expire() decay, $track checks)*
- [x] 163. **Vehicle fuel** [now] — vehicles consume fuel; refueling stations and running dry. *(consumption attrs, refuel services)*
- [x] 164. **Small spaceship** [now] — cockpit, cabin, airlock; launch, fly between sites, dock. *(multi-room vehicles, interlocks, docking = relinking — capstone)*

## 15. Building & World Tools (165–175)

- [x] 165. **Prototype library** [now] — organize spawnable content with inheritance and tags. *(spawner prototypes, @parent, @clone, pack data)*
- [x] 166. **Batchcode areas** [now] — build a whole zone from a repeatable build file. *(@export/@import area files, plan → apply diffs)*
- [x] 167. **Random dungeon generator** [now] — spin up a connected derelict-ship layout on demand. *(softcode room/exit carving, instance templates, teardown)*
- [x] 168. **Room templates** [now] — stamp out consistent rooms (corridor, quarters) fast. *(@clone/@parent templates, $stamp wizards)*
- [x] 169. **Zone mass-edit** [now] — retag, redescribe, or audit every room in a zone at once. *(@find/@foreach, zone_rooms(), dry-run discipline)*
- [x] 170. **Builder wizard** [now] — menu-driven room/exit creation for non-coders. *(prompt() building wizards, create_obj + exit wiring)*
- [x] 171. **Dynamic descriptions** [now] — descs that weave in state (weather, damage, time). *([[...]] inline evaluation — REALM's flagship)*
- [x] 172. **World audit report** [now] — find orphaned objects, broken exits, oversized attributes. *(@eval, search_world introspection, health queries)*
- [x] 173. **CSV world import** [now] — bulk-load rooms/items/NPCs from structured data. *(area files, @import stable-id sync, realm import CLI)*
- [x] 174. **Auto-map generator** [now] — render an ASCII map computed from actual room links. *(exits() graph BFS, sandboxed-Python rendering)*
- [x] 175. **Player housing customization** [now] — players redescribe and furnish owned rooms safely. *(@chown, control locks, room-owned $-commands, safe attr flags)*

## 16. Admin, Moderation & Staff Tools (176–186)

- [x] 176. **Staff dashboard** [now] — one command showing server health, players, recent errors. *(@stats, who, @eval ops queries)*
- [x] 177. **Jail system** [now] — timeout misbehaving players with auto-release and logging. *(teleport_obj, exit locks, wait() release, log attrs)*
...
- [x] 179. **Character approval queue** [now] — staff review/approve chargen submissions. *(unapproved tags, gating wards, $approve + pemit)*
...
- [x] 181. **Announcement system** [now] — formatted server-wide notices with history. *(remit loops, history attrs, opt-out filters)*
- [x] 182. **Object snapshot/restore** [now] — back up and restore a single object's attributes. *(@clone backups, @export, @eval attr dumps)*
- [x] 183. **Permission tiers in practice** [now] — a worked tour of roles, locks, and quelling. *(god/admin/builder/player roles, controls(), quell)*
- [x] 184. **New-player onboarding** [now] — auto-greeting, mentor assignment, starter kit. *(world-master ON_CONNECT, first-login attrs, kit spawning)*
...
- [x] 186. **Watchlist & alerts** [now] — staff alerts when flagged players log in or act. *(ON_CONNECT watchers, watched tags, staff pemits)*
...
- [x] 189. **In-room minimap** [now] — small ASCII map of nearby rooms shown on look. *(ON_LOOK pemit, exits() BFS, exploration-memory attrs)*
- [x] 190. **Score screen** [now] — attractive at-a-glance character status. *($score, eval_attr layout functions)*
- [x] 191. **Help system extensions** [now] — auto-generated help from command metadata. *(the built-in help pipeline: categories, usage, help_text)*
- [ ] 192. **Clickable links** [small] — clickable exits, items, and commands where supported. *(gap G11: MXP markup at the telnet edge)*
- [x] 193. **GMCP/OOB data** [now] — push vitals and room info to modern clients (Mudlet). *(oob(), telnet GMCP, websocket parity)*
...
- [x] 196. **Personal aliases (nicks)** [now] — players define their own command/object shorthands. *(carried alias gadgets: inventory $-commands)*
...
- [x] 198. **Quest framework** [now] — quest objects with stages, journal, and completion hooks. *(admin-owned quest masters, stage attrs, $quests journal)*
- [x] 199. **Delivery quest** [now] — fetch/carry template with timers and failure states. *(ON_GIVE verification, deadline timestamps)*
- [x] 200. **Collection counters** [now] — "salvage 5 relays" tracked automatically on acquisition. *(zone-master ON_GET counting, tagged objectives)*
- [x] 201. **Branching quest** [now] — choices that lock/unlock different endings. *(prompt() branches, mutually exclusive state attrs)*
- [x] 202. **World event: invasion** [now] — staged zone-wide event with phases and cleanup. *(zone-master phases, spawner waves, ON_RESET cleanup)*
- [x] 203. **Cutscenes** [now] — timed, paced sequences of text shown to one or many players. *(wait()-paced pemit/remit, $skip via cancel_wait)*
- [x] 204. **GM possession tools** [now] — staff speak/act through any NPC live in a scene. *(@force, control locks, puppet forwarding)*
- [x] 205. **Scene logger** [now] — opt-in RP scene recording with pose order tracking. *(recorder ^listen + ON_POSE, consent attrs, $export)*
- [x] 206. **Rumor mill** [now] — plant rumors that spread organically via NPCs over days. *(rumor attrs hopping via on_tick, ^listen pickup, decay)*
- [x] 207. **Achievements** [now] — badges for milestones with hidden and progressive tiers. *(ON_* watchers, badge attrs, secret attr flags)*
- [x] 208. **Collectible lore** [now] — scattered logs/books that unlock codex entries when found. *(ON_GET/ON_USE unlocks, $codex rendering)*

## 19. Puzzles & Mechanisms (209–218)

- [x] 209. **Lever combination** [now] — pull levers in the right pattern to open the vault. *(ON_PUSH, shared pattern attrs on a controller object)*
- [x] 210. **Keypad code** [now] — enter a code discovered elsewhere in the world. *(prompt() entry, secret attrs, hint placement)*
- [x] 211. **Riddle door** [now] — answer free-text riddles with fuzzy answer matching. *(prompt() free text, normalization with string functions)*
- [x] 212. **Weight-plate puzzle** [now] — place the right objects on pressure plates. *(ON_RECEIVE/ON_PUT contents sensing)*
- [x] 213. **Power routing puzzle** [now] — reroute ship power through junctions to unlock a deck. *(graph-state attrs, eval_attr win checks)*
- [x] 214. **Simon sequence** [now] — repeat a growing pattern of signals correctly. *(wait()-chained signals, prompt() echo-back, timing windows)*
- [x] 215. **Shifting maze** [now] — maze whose exits rearrange on a timer or trigger. *(on_tick exit relinking, mutable topology)*
- [x] 216. **Escape room** [now] — locked suite of puzzles against a countdown, resettable. *(enter_instance() suites — reset is a fresh instance)*
- [x] 217. **Hidden object search** [now] — search-the-room mechanics with Per-based finds. *(concealment tags, the search command, seeded secrets)*
- [x] 218. **Puzzle reset engineering** [now] — making any puzzle safely repeatable for the next group. *(ON_RESET/zone_reset, instances, $reset scripts)*

## 20. Social & Player Systems (219–229)

- [x] 219. **Friends list** [now] — contacts with login/logout notifications and privacy opt-outs. *(world-master ON_CONNECT/ON_DISCONNECT, consent attrs)*
- [x] 220. **Titles & badges** [now] — earned display titles shown in look/finger. *(title attrs, desc_extras rendering)*
- [x] 221. **Player organizations** [now] — guilds/crews with ranks, invites, and rosters. *(org master objects, rank attrs, lock expressions)*
- [x] 222. **Org treasury & storage** [now] — shared bank and lockers with rank-gated access. *(org credits, rank-gated $-commands, locked containers)*
- [x] 223. **Elections** [now] — org leadership voting with terms, ballots, and tallies. *(ballot attrs, vote dedupe, on_tick tallies)*
- [x] 224. **Petition/ticket system** [now] — players file requests; staff triage a queue. *(ticket-queue attrs, $claim/$resolve, pemit notifications)*
- [x] 225. **Player-to-player notes** [now] — staff annotations and player-visible profiles. *(notes attrs with secret/visual attr flags)*
- [x] 226. **Mentor program** [now] — veterans flagged to help newbies, with matchmaking. *(mentor tags, pairing attrs, ON_CONNECT nudges)*
- [x] 227. **Event calendar & RSVP** [now] — players schedule events; reminders fire. *(event attrs, $rsvp, on_tick reminders)*
- [x] 228. **Leaderboards** [now] — top crafters/fighters/richest, computed periodically. *(on_tick aggregation, search_world scans, cached boards)*
- [x] 229. **Login streak rewards** [now] — daily/anniversary perks with catch-up grace. *(ON_CONNECT date math, streak attrs, perk spawning)*
...
- [x] 235. **JSON content packs** [now] — load item/NPC definitions from versioned JSON files. *(@pack, realm pack import, pack authoring)*
...
- [x] 244. **Player macros** [now] — record and replay short command sequences with safeguards. *(personal alias gadgets, multi-line scripts, sandbox limits)*
- [x] 245. **Event bus tour** [now] — subscribing to and emitting game events across systems. *(two-pass propagation, act() custom events, action tags)*
- [x] 246. **Hot-reload workflow** [now] — the live-world dev loop: what updates instantly, what doesn't. *(softcode is always live; @reload for data rules)*
- [x] 247. **Testing your game** [now] — scripted tests for triggers, behaviors, and areas. *(the realm.testing Simulator, pytest, virtual-clock waits)*
- [x] 248. **Custom input handling** [now] — capture raw input for a minigame, code entry, or editor. *(prompt()/choose()/confirm() wizards, persistent prompts)*
...
- [x] 252. **Invent an action** [now] — fire a custom act() event with a payload, subscribe to it, and intercept it in flight. *(act() extra/tags, adata subscribers, on_check set_adata and block, behavior_def filters)*
